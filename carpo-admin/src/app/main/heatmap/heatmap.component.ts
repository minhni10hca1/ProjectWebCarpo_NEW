...
export class HeatmapComponent implements OnInit {
  ...
  mapZoom: number = 12;
  public filterCampaignCars: any[];
  public customerCampaigns: any[]; //ds combo hợp đồng
  constructor(
  ...
  ngOnInit() {
    this.customerCampaigns = JSON.parse(localStorage.getItem(SystemConstants.CURRENT_USER_CAMPAIGN));
    this.initMap();
  }
  public initMap() {
  ...
  public getPointsByDeviceIds(cars: any, loc_lat: any, loc_lng: any) {
    //7 ngày trước
    ...
    // let startDate = moment().subtract(29, 'd').format('YYYY-MM-DD');
    //end
    var deviceIds = cars.split(',').map(function (n) {
      return String(n);
    });
    ...
    this._dataService.post('/trackings/getPointsHeatMap', JSON.stringify(paramOptions)).subscribe((response: any) => {
      console.log('response.data.length' , response.data.length);
      var centerlat = parseFloat(loc_lat);
      var centerlng = parseFloat(loc_lng);
      this.map = new google.maps.Map(document.getElementById('map'), {
        zoom: this.mapZoom,
      ...
      this.heatmap = new google.maps.visualization.HeatmapLayer(heatmapOpts);
      if (response.success == 1 && response.data.length > 0) {
        var latLngs = [];
        response.data.forEach(function (item) {
            latLngs.push({location: new google.maps.LatLng(item.location_lat, item.location_long), weight: 1});
        });
